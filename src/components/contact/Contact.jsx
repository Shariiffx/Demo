...
const Contact = () => {
    ...
    useEffect(() => {
        emailjs.init('service_atl9sgz'); // Replace 'YOUR_USER_ID' with your actual user ID
    }, []);

    const sendEmail = (e) => {
        e.preventDefault();

        emailjs.sendForm('service_atl9sgz', 'template_uv83z4c', formRef.current, 'GCsvDY6kXdLSJDxFE')
            .then(
                (result) => {
                    setSuccess(true);
                    setError(false);
        ...
        >
            <motion.div className="textContainer" variants={variants}>
                ...
                <motion.div className="item" variants={variants}>
                    <h2>Mail</h2>
                    <span>khanshariffy786@gmail.com</span>
                </motion.div>
                <motion.div className="item" variants={variants}>
                    <h2>Address</h2>
            ...
            <div className="formContainer">
                ...
                >
                    ...
                    <textarea id="message" rows={8} placeholder="Message" name="message"></textarea>
                    <button type="submit">Submit</button>
                    {error && <span style={{ color: 'red' }}>Error sending email. Please try again.</span>}
                    {success && <span style={{ color: 'green' }}>Email sent successfully!</span>}
                </motion.form>
            </div>
        </motion.div>
    );
